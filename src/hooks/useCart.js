...
const useCart = () => {
  ...
  useEffect(() => {
    fetch(`https://wicked-mausoleum-90619.herokuapp.com/cart/${uid}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.length) {
  ...
  function addToCart(course) {
    ...
    if (isHave) {
      ...
      fetch("https://wicked-mausoleum-90619.herokuapp.com/courses/add", {
        method: "post",
  ...
  function remove(id) {
    fetch(`https://wicked-mausoleum-90619.herokuapp.com/delete/${id}`, {
      method: "delete",
    })
      .then((res) => res.json())
      .then((data) => {
